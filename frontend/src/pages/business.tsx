...
import { FaPlus, FaUserTie, FaClipboardList, FaProjectDiagram } from 'react-icons/fa';
...
const BusinessProfilePage: React.FC = () => {
  ...
  const [professors] = useState<Professor[]>([
    { id: '1', name: 'Dr. Jane Smith', expertise: 'Artificial Intelligence', projects: 3 },
    { id: '2', name: 'Prof. John Doe', expertise: 'Data Science', projects: 5 },
    { id: '3', name: 'Dr. Emily Brown', expertise: 'Cybersecurity', projects: 2 },
  ]);
  const [projects] = useState<Project[]>([
    { id: '1', title: 'AI-driven Customer Service', description: 'Developing an AI chatbot for customer support', professor: 'Dr. Jane Smith', status: 'In Progress' },
            ...
            <CardContent>
              <ul className="space-y-4">
                {professors.map((professor) => (
                  <li key={professor.id} className="flex justify-between items-center">
                    <div>
                      <h3 className="font-semibold">{professor.name}</h3>
                      <p className="text-sm text-gray-600">{professor.expertise}</p>
                    </div>
                    <span className="text-sm text-blue-600">{professor.projects} projects</span>
                  </li>
                ))}
              </ul>
            </CardContent>
          ...
          <CardContent>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {projects.map((project) => (
                <Card key={project.id} className="cursor-pointer hover:shadow-lg transition-shadow" onClick={() => navigate(`/project/1`)}>{/*to be changed*/}
                  <CardContent className="p-4">
                    <h3 className="font-semibold mb-2">{project.title}</h3>
                    <p className="text-sm text-gray-600 mb-2">{project.description}</p>
                    <div className="flex justify-between items-center">
                      <span className="text-sm text-blue-600">{project.professor}</span>
